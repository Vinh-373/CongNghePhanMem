 ...
 * - school: {lat,lng} - vị trí trường học
 * - zoom: số zoom mặc định
 * - setRealPolyline: Hàm để gửi polyline thật lên Dashboard
 */
const LeafletRoutingMap = ({
  routes = [],
  buses = [],
  ...
  zoom = 13,
  defaultCenter = { lat: 10.776, lng: 106.702 },
  setRealPolyline,    // ⭐ THÊM PROP NHẬN POLYLINE THẬT
}) => {
  const mapRef = useRef(null);
  const routeControlsRef = useRef([]);
  const markersRef = useRef([]);
  ...
  useEffect(() => {
    ...
    routes.forEach((route, i) => {
      ...
      // ⭐ LẤY POLYLINE THẬT KHI LEAFLET ROUTING MÁY TÍNH XONG
      control.on("routesfound", function (e) {
        const realPolyline = e.routes[0].coordinates.map((c) => ({
          lat: c.lat,
          lng: c.lng,
        }));
        console.log("Polyline thật từ Leaflet:", realPolyline);

        // Gửi lên Dashboard
        if (setRealPolyline) setRealPolyline(realPolyline);
      });
    ...
    if (bounds.isValid()) map.fitBounds(bounds);
  }, [routes, buses, school, zoom, defaultCenter, setRealPolyline]);
