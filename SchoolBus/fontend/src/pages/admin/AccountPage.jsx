...
// Loại bỏ import Separator vì không cần thiết cho layout side-by-side
import { User, KeyRound, Mail, Phone, Loader2, FilePenLine, XCircle } from "lucide-react";
...
const getFullAvatarUrl = (relativeUrl) => {
    ...
    // Tránh trùng lặp /schoolbus/ nếu đường dẫn đã có sẵn
    if (relativeUrl.startsWith('/uploads')) {
        return `${API_BASE_URL}${relativeUrl}`;
    }
    // Trường hợp khác (ví dụ: đang dùng đường dẫn cũ hoặc đường dẫn tuyệt đối)
    return relativeUrl;
};

export default function AccountPage() {
    const { user, setUser } = useOutletContext(); // Lấy hàm setUser để cập nhật state sau khi đổi profile
    ...
    const handleSaveProfile = async () => {
        ...
        // fd.append("email", userData.email); // Email thường không được phép sửa
        ...
        if (selectedFile) {
            ...
            // Nếu không có file mới, nhưng người dùng đã có ảnh, gửi lại đường dẫn cũ
        ...
        try {
            ...
            // Cập nhật lại state user gốc trong OutletContext
            if (setUser) {
                // Giả định response trả về userInfo
                setUser({ 
                    ...user, 
                    name: res.data.updatedUser.hoten,
                    phone: res.data.updatedUser.sodienthoai,
                    avatarUrl: res.data.updatedUser.anhdaidien, // Sử dụng đường dẫn mới từ server
                });
            }
...
const userStr = localStorage.getItem("user");
    ...
    // === Layout Chính: Sử dụng Grid cho 2 cột trên màn hình lớn ===
    return (
        <div className="max-w-6xl mx-auto ">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {/* Cột 1: Thông tin cá nhân (2/3 width on large screens) */}
                <Card className="shadow-xl border-t-4 border-[#175e7a] lg:col-span-2">
                    ...
                    <CardContent>
                        <div className="flex flex-col xl:flex-row gap-6">
                            {/* Avatar & Role (w-full trên mobile/tablet, w-1/3 trên desktop) */}
                            ...
                            {/* Form chi tiết (w-full trên mobile/tablet, w-2/3 trên desktop) */}
                                ...
                                <div className="flex justify-end pt-3">
                                    {!isEditing ? (
                                        <Button
                                            onClick={() => {
                                                setIsEditing(true);
                                                setSelectedFile(null); // Reset file khi bắt đầu edit
                                            }}
                                            variant="outline"
                                        ...
                                        <div className="flex gap-2">
                                            <Button 
                                                onClick={() => {
                                                    setIsEditing(false);
                                                    // Reset lại data về trạng thái ban đầu của user prop
                                                    if (user) {
                                                        setUserData({ 
                                                            name: user.name, 
                                                            email: user.email, 
                ...
                {/* Cột 2: Thay đổi mật khẩu (1/3 width on large screens) */}
                {/* Không cần col-span, nó tự động chiếm cột còn lại (1/3) */}
