...
// =================================================================
// ============== HÀM HỖ TRỢ TẢI DỮ LIỆU ===========================
// =================================================================

const fetchRoutesAndPoints = async (setRoutesData, setTotalStops, setStopOptions, setLoading) => {
    try {
        setLoading(true);
...
export default function RoutesPage() {
    ...
    useEffect(() => {
        fetchRoutesAndPoints(setRoutesData, setTotalStops, setStopOptions, setLoading);
    }, []);
    ...
    // --- Hàm xử lý THÊM TUYẾN MỚI ---
    const handleAddNewRoute = async (formData) => {
        try {
            // Chuyển đổi dữ liệu trước khi gửi
            const dataToSend = { ...formData };

            if (dataToSend.dsdiemdung && Array.isArray(dataToSend.dsdiemdung)) {
                const stopIds = dataToSend.dsdiemdung.map(item => {
                    return typeof item === 'object' ? item.value : item;
                });
                dataToSend.dsdiemdung = JSON.stringify(stopIds);
            }

            dataToSend.trangthai = String(dataToSend.trangthai);
    ...
    const handleUpdateRoute = async (formData) => {
        try {
            ...
            const res = await axios.put(
                `http://localhost:5001/schoolbus/admin/update-route/${editingRoute.idtuyenduong}`,
                dataToSend
            );
    ...
    // --- Hàm xử lý XÓA MỀM TUYẾN ---
    const handleSoftDeleteRoute = async (routeId, routeName) => {
        if (!window.confirm(`Bạn có chắc chắn muốn XÓA MỀM tuyến đường "${routeName}" không? Tuyến sẽ bị chuyển về trạng thái Tạm dừng (0).`)) {
            return;
        }

        try {
            await axios.put(`http://localhost:5001/schoolbus/admin/delete-route/${routeId}`);
            toast.success(`Xóa mềm tuyến "${routeName}" thành công!`);
            refreshData();
        } catch (error) {
            console.error("❌ Lỗi xóa mềm:", error);
            toast.error(error.response?.data?.message || "Xóa mềm thất bại!");
        }
    };
    ...
    // =================================================================
    // ============== FIELDS VÀ INITIAL DATA ===========================
    // =================================================================

    // --- Fields cơ bản (không có initialValue) ---
    const addRouteFields = useMemo(() => [
        { name: "tentuyen", label: "Tên tuyến đường", type: "text", placeholder: "Ví dụ: Tuyến Quận 1 - Quận 7", required: true },
        { name: "mota", label: "Mô tả lộ trình", type: "text", placeholder: "Mô tả ngắn gọn...", required: false },
        { name: "loaituyen", label: "Loại tuyến", type: "select", options: [{ value: "Đón", label: "Đón học sinh" }, { value: "Trả", label: "Trả học sinh" }], defaultValue: "don" },
        ...
        { name: "dsdiemdung", label: "Danh sách điểm dừng", type: "multi-select", options: stopOptions, required: false }
    ], [stopOptions]);

    // --- Chuẩn bị initialData cho dialog sửa ---
    const getInitialDataForEdit = (route) => {
        let initialStopIds = [];

        try {
            if (route.dsdiemdung) {
                const stopIds = JSON.parse(route.dsdiemdung);
                // Chuyển ID thành string để match với select options
                initialStopIds = stopIds.map(id => String(id));
            }
        } catch (e) {
            console.error("Lỗi parse dsdiemdung:", e);
        }

        return {
            tentuyen: route.tentuyen || "",
            mota: route.mota || "",
            loaituyen: route.loaituyen || "Đón",
            trangthai: String(route.trangthai) || "1",
            dsdiemdung: initialStopIds,
        };
    };

    // =================================================================
    // ============== LOGIC VÀ VIEW CƠ BẢN =============================
    // =================================================================
    ...
    const busesData = useMemo(() => [], [selectedRoutes]);
    ...
    return (
        <div className="space-y-6">
                        ...
                        <TableBody>
                            {routesData.map(route => (
                                <TableRow key={route.idtuyenduong}>
                                    ...
                                    <TableCell>{route.loaituyen === 'don' ? 'Đón' : 'Trả'}</TableCell>
                                    ...
                                    <TableCell>
                                        <div className="flex justify-end gap-2">
                                            ...
                                            {/* SỬA */}
                                            <Button variant="outline" size="icon" onClick={() => handleEditRouteClick(route)}>
                                            ...
                                            <Button
                                                ...
                                                onClick={() => handleSoftDeleteRoute(route.idtuyenduong, route.tentuyen)}
                                            >
            ...
            {editingRoute && (
                <AddEntityDialog
                    key={editingRoute.idtuyenduong}
                    isOpen={isEditDialogOpen}
                    onClose={() => {
                        setIsEditDialogOpen(false);
                        setEditingRoute(null);
                    }}
                    title={`Sửa Tuyến: T-${editingRoute.idtuyenduong.toString().padStart(3, "0")}`}
                    description="Cập nhật thông tin tuyến đường và danh sách điểm dừng."
                    fields={addRouteFields}
                    initialData={getInitialDataForEdit(editingRoute)}
